...
class Empty extends TweetSet :
  ...
  def mostRetweeted = throw NoSuchElementException("head of EmptyList")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet :
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if p(this.elem) then this.left.filterAcc(p, this.right.filterAcc(p, acc.incl(this.elem)))
  ...
  def mostRetweeted: Tweet =
    def compareRetweets(t1: Tweet, t2: Tweet): Tweet =
      if t1.retweets > t2.retweets then t1 else t2

    if this.left.isEmpty && this.right.isEmpty then this.elem
    else if this.left.isEmpty then compareRetweets(this.elem, this.right.mostRetweeted)
    else if this.right.isEmpty then compareRetweets(this.elem, this.left.mostRetweeted)
    else compareRetweets(this.elem, compareRetweets(this.left.mostRetweeted, this.right.mostRetweeted))
...
object Nil extends TweetList :
  def head = throw NoSuchElementException("head of EmptyList")

  def tail = throw NoSuchElementException("tail of EmptyList")
