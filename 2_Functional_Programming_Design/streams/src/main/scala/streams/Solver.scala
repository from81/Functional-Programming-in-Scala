...
trait Solver extends GameDef:
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    // neighbors.filter((block, moves) => !explored.contains(block))
    for {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if initial.isEmpty then LazyList.empty
      ...
      initial.head #:: from(
        initial.tail ++ neighbors, // we want LazyList of neighbors sorted by distance in ascending order
        explored ++ neighbors.map(item => item._1)
      )
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(
      LazyList((startBlock, List.empty[Move])),
      Set(startBlock)
    )
